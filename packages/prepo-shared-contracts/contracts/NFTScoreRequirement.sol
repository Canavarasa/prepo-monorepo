...
import {IERC721, INFTScoreRequirement} from "./interfaces/INFTScoreRequirement.sol";
...
contract NFTScoreRequirement is INFTScoreRequirement {
  ...
  function _satisfiesScoreRequirement(address account)
    internal
    view
    virtual
    returns (bool)
    ...
    override
  {
    ...
    emit CollectionScoresChange(
      collections,
      new uint256[](collections.length)
    );
  }
  ...
  function getCollectionScore(IERC721 collection)
    ...
    returns (uint256)
  {
    if (_collectionToScore.contains(address(collection))) {
      return _collectionToScore.get(address(collection));
    }
    return 0;
  }

  function getAccountScore(address account)
    public
    view
    virtual
    override
