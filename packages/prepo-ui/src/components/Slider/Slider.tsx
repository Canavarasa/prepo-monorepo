...
import React, { useEffect, useCallback, useState, useRef, useMemo, ComponentType } from 'react'
...
const ARow: ComponentType<RowProps> = dynamic(() => import('antd').then(({ Row }) => Row))
...
const Container = styled(ReactSlider)<{ labelPosition: LabelPosition; thickness?: Thickness }>`
  display: block;
  width: 100%;
  ${({ thickness }) => getResponsiveHeight(thickness)}
`
