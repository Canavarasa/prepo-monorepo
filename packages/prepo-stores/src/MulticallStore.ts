...
function generateUniqueCallId(context: ContractCallContext): string | undefined {
  const firstCall = context.calls[0]
  if (!firstCall) return undefined
  return `${context.reference}${firstCall.methodName}${JSON.stringify(firstCall.methodParameters)}`
}
...
export class MulticallStore {
        ...
        this.multicall
          ...
          .catch((error) => {
            ...
            if (error.code === 'CALL_EXCEPTION') {
              ...
              // do not throw read value errors in production, read values don't handle errors well
  ...
  addCall(context: ContractCallContext): void {
    ...
    if (uniqueCallId === undefined || this.activeCalls.has(uniqueCallId)) return

    // Get contract reference space to add call to
    const contractIndex = this.contractCallContexts.findIndex(
      (_context) => _context.contractAddress === context.contractAddress
    )
  ...
  removeCall(contextToRemove: ContractCallContext): void {
    ...
    if (uniqueCallIdToRemove === undefined) return
    ...
    // Remove the call
    if (this.contractCallContexts[contractIndex]) {
