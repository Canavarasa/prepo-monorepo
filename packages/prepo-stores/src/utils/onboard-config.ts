...
import { Network, NETWORKS, WALLETCONNECT_PROJECT_ID } from 'prepo-constants'
...
export const getOnboardConfig = async (
  ...
  const [
    ...
    { default: walletConnectModule },
    { default: coinbaseWalletModule },
    { default: gnosisModule },
  ] = await Promise.all([
  ...
  const walletConnect = walletConnectModule({
    ...
    requiredChains: [NETWORKS.arbitrumOne.chainId],
    version: 2,
  })
  const coinbaseWallet = coinbaseWalletModule({ darkMode: false })
  const safeWallet = gnosisModule()

  return {
    wallets: [injected, safeWallet, coinbaseWallet, walletConnect],
    chains: supportedChains.map(({ chainId, chainName, rpcUrls, blockExplorer }) => ({
      blockExplorerUrl: blockExplorer,
      id: chainIdToHexString(chainId),
      label: chainName,
      rpcUrl: getRpcOverrides()[chainId] ?? rpcUrls[0],
      token: 'ETH',
    })),
    appMetadata: {
      name: appName,
