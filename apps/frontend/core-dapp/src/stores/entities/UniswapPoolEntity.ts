...
import { fraction, Fraction } from '../../utils/fraction-utils'
...
export class UniswapPoolEntity extends ContractStore<RootStore, SupportedContracts> {
  ...
  get feeAsFraction(): Fraction | undefined {
    const fee = this.fee()?.[0]
    if (fee === undefined) return undefined
    return fraction(BigNumber.from(fee), BigNumber.from(1_000_000))
  }
}
