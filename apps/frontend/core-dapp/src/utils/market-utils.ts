...
const filterAndBackfillData = (
  ...
  poolData.forEach(({ liquidity, token0, token1, timestamp }) => {
    ...
    // use mid of payout range if price is 0 (no data)
    let longTokenPrice = (payoutRange[0] + payoutRange[1]) / 2
    if (tokenPrice > 0) {
      longTokenPrice = tokenType === 'long' ? tokenPrice : 1 - tokenPrice
    }
...
export const formatMarketHistoricalData = (
  ...
  if (!shortTokenPool && !longTokenPool)
    return {
      long: [],
      short: [],
    }
  ...
  return {
    long: Object.values(longPoolData),
    short: Object.values(shortPoolData),
  }
}
...
export const normalizePoolsData = (poolsData?: PoolsData): NormalizedPoolsData | undefined => {
  if (!poolsData?.longTokenPool || !poolsData.shortTokenPool) return undefined
...
const syncLatestData = (
  interval: number,
  existing: MarketHistoryData[],
  newData?: MarketHistoryData[]
): MarketHistoryData[] => {
  const output = existing
  if (!newData || newData.length < 1) return existing
  const latestHistoricalData = output[output.length - 1]
  ...
  if (sameWindow) {
    output[output.length - 1] = latestData
  } else if (newWindow && withinInterval) {
    output.push(latestData)
  }

  return output
}

export const syncPoolsData = (
  historicalDatas: SeparatedMarketHistoryData,
  type: HistoricalDataQueryType,
  latestDatas?: SeparatedMarketHistoryData
): SeparatedMarketHistoryData => {
  const interval = getIntervalInUnix(type)
  const long = syncLatestData(interval, historicalDatas.long, latestDatas?.long)
  const short = syncLatestData(interval, historicalDatas.short, latestDatas?.short)
