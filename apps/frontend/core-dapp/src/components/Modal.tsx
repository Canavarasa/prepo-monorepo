...
const AModal: ComponentType<ModalProps> = dynamic(() => import('antd').then(({ Modal }) => Modal))
...
export const modalStylesCss = css<Props>`
  &&& {
    ${({ bottom }) => (bottom ? modalPositionBottomCss : null)}
    .ant-modal-content {
      background-color: ${({ theme }) => theme.color.neutral9};
      border-radius: ${({ theme }) => theme.borderRadius.lg};
      padding: 0 ${spacingIncrement(10)};
      overflow: hidden;
      ${({ bottom }) => (bottom ? modalPositionBottomCss : null)}
      ${({ bottom }) => (bottom ? bottomModalBorderCss : null)}
    }
    .ant-modal-header {
