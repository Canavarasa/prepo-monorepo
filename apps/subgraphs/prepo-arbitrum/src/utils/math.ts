...
export function tenToThePowerOf(decimals: BigInt): BigDecimal {
...
export function convertTokenToDecimal(tokenAmount: BigInt, exchangeDecimals: BigInt): BigDecimal {
  ...
  return tokenAmount.toBigDecimal().div(tenToThePowerOf(exchangeDecimals))
}
...
export function sqrtPriceX96ToTokenPrices(
  ...
  const price1 = num
    .div(denom)
    .times(tenToThePowerOf(token0Decimals))
    .div(tenToThePowerOf(token1Decimals))
